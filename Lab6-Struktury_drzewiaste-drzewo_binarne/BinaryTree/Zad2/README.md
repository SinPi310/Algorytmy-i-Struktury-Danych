...
Napisz, wzorując się na metodzie `Print<T>()`, metodę o sygnaturze:

``` C#
public static void PrintTree<T>(BinTreeNode<T> p, int level = 0)
```

która wypisuje na konsolę wartości węzłów drzewa w sposób przypominający prezentowanie struktury folderów i plików w systemie operacyjnym. Sugeruj się podanym przykładem.
